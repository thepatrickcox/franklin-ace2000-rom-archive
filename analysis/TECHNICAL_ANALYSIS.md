...
A note on stray content in unused space: these four parts are development betas that were erased and reprogrammed many times during firmware work, so bytes in zero-fill and inter-routine gaps cannot be cleanly attributed — apparent fragments may be residue from earlier, unrelated programming cycles rather than part of this firmware. No coherent assembler source or developer comments were identified, but given the rewrite history this is not asserted as a clean negative. (For contrast, another researcher found an assembler source fragment with developer comments in an unused region of the production ACE 500 firmware; the development nature of these parts makes such a comparison inexact.)
