...
A note on stray content in unused space: these four parts are development betas that were erased and reprogrammed repeatedly during firmware work. Because EPROM programming can only clear bits (1 to 0) and only bulk UV erasure restores them, a later write that does not fully erase or fully cover an earlier one can leave fragments of a previous programming intact in regions the current firmware does not use. Bytes in zero-fill and inter-routine gaps therefore cannot be cleanly attributed: apparent fragments may be residue from earlier, unrelated cycles rather than part of this firmware.

Two such fragments were identified, both coherent 65C02 code stranded in otherwise zero-filled regions and not referenced by any JSR/JMP in the surrounding firmware:

* P2 (companion ROM), $C0D0: `STA $CFFF / STA $C079 / RTS` — a three-instruction soft-switch helper (expansion-ROM disable, then a IIe bank switch), surrounded by zero-fill at $C0C0 and $C0E0. No call site targets $C0D0 anywhere in P2.
* P3 (disk firmware), $06A6: `LDY #$56 / STY $3C / SEC / CLV / JMP $CF16` — a short setup-and-jump into the $C800-$CFFF expansion-ROM window, isolated in zero-fill between $0690 and $06B0.

Both are too well-formed to be random noise (each disassembles cleanly into sensible instructions with valid soft-switch and zero-page addresses) yet are unreachable from the live code. Coherent-but-orphaned code in dead space is the expected signature of a partial overwrite on a repeatedly reprogrammed development part. This is offered as the most consistent interpretation, not a certainty; no developer comments or assembler source text were found in any image. (For contrast, another researcher found an assembler source fragment with comments in an unused region of the production ACE 500 firmware; the development nature of these parts makes a direct comparison inexact.)
